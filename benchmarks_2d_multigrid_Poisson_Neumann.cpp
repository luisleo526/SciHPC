    ...
    for (int i = 0; i < phi.scalar->nx; ++i) {
        for (int j = 0; j < phi.scalar->ny; ++j) {
            mg.at[0]->rhs[mg.at[0]->of(i, j)] = -2.0 * M_PI * M_PI * sin(M_PI * geo.xc[i]) * sin(M_PI * geo.yc[j]) -
                                                50.0 * M_PI * M_PI * sin(5.0 * M_PI * geo.xc[i]) *
                                                sin(5.0 * M_PI * geo.yc[j]);
            auto index = phi.scalar->index_mapping(i + 1, j + 1, 1);
            phi.scalar->data[index.i][index.j][index.k] = sin(M_PI * geo.xc[i]) * sin(M_PI * geo.yc[j]) +
                                                          sin(5.0 * M_PI * geo.xc[i]) * sin(5.0 * M_PI * geo.yc[j]);
        }
    }
